...
module Entity
    ...
    # Define the serialize method for HllSet{10}
    function Serialization.serialize(io::IO, hll::HllSet{10})
        # Add your serialization logic here
        # For example, write the fields of HllSet{10} to the IO stream
        write(io, hll.field1)
        write(io, hll.field2)
    ...
    # Operation
    struct Operation{FuncType,P}
        op::FuncType
        args::Vector{HllSet{P}}
    end
    ...
    function op_args(op::Union{Operation, Nothing}) 
        if op == nothing
            ...
            return [HllSets.id(hll) for hll in op.args]
        end
    end
    ...
    function store_entity(r::PyObject, hll::HllSets.HllSet{P}; grad=0.0, op=nothing,  prefix::String="b") where P
        ...
        return r.fcall("store_entity", 1, prefix, sha1, dataset, 0, op_op(op), args), sha1
    end
    ...
    mutable struct Instance{P}
        ...
        # function Instance{P}(::Colon,  hll::HllSets.HllSet{P}; grad=0.0, op=nothing, prefix::String="b", r::PyObject=r) where {P}
        #     response, sha1 = store_entity(r, hll, grad=grad, op=op, prefix=prefix)
        #     new{P}(sha1, hll, grad, op)
        # end
    ...
    function show(io::IO, instance::Instance)
        ...
        # println(io, "Instance(", sha1_str, "; ", hll_str, "; ", grad_str, "; ", op_str, ")")
        println(io, "\nInstance(sha1: ", sha1_str, ";\n hll_count: ", HllSets.count(instance.hll), ";\n grad: ", grad_str, ";\n op: ", op_str, ");\n")
    end
    ...
    function union(r::PyObject, a::Instance{P}, b::Instance{P}) where {P}
        ...
        op_result = Operation(union, [a.hll, b.hll])
        return Instance{P}(r, hll_result; grad=0.0, op=op_result)
    end

    # union backprop
    function backprop!(instance::Instance{P}, 
            instance_op::Union{Operation{FuncType, ArgTypes}, Nothing}=instance.op) where {P, FuncType<:typeof(union), ArgTypes}
    ...
    # intersect backprop
    function backprop!(instance::Instance{P}, instance_op::Operation{FuncType, ArgTypes}) where {P, FuncType<:typeof(intersect), ArgTypes}
    ...
    # xor backprop
    function backprop!(instance::Instance{P}, instance_op::Operation{FuncType, ArgTypes}) where {P, FuncType<:typeof(xor), ArgTypes}
    ...
    function comp(r::PyObject, a::Instance{P}, b::Instance{P}; opType=comp) where {P}
        ...
        return Instance{P}(r, hll_result; grad=comp_grad, op=op_result)
    end
    ...
    function backprop!(instance::Instance{P}, instance_op::Operation{FuncType, ArgTypes}) where {P, FuncType<:typeof(comp), ArgTypes}
    ...
    # added backprop
    function backprop!(instance::Instance{P}, instance_op::Operation{FuncType, ArgTypes}) where {P, FuncType<:typeof(added), ArgTypes}
    ...
    # deleted backprop
    function backprop!(instance::Instance{P}, instance_op::Operation{FuncType, ArgTypes}) where {P, FuncType<:typeof(deleted), ArgTypes}
    ...
    # retained backprop
    function backprop!(instance::Instance{P}, instance_op::Operation{FuncType, ArgTypes}) where {P, FuncType<:typeof(retained), ArgTypes}
    ...
    function backprop!(instance::Instance{P}, instance_op::Operation{FuncType, ArgTypes}) where {P, FuncType<:typeof(advance), ArgTypes}
